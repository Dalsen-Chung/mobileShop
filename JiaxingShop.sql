...
CREATE TABLE `jx_order`  (
  ...
  `status` tinyint(1) NULL DEFAULT NULL COMMENT '订单状态，1支付成功，2已发货，3已取消',
  `addtime` int(12) NULL DEFAULT NULL COMMENT '添加时间',
  PRIMARY KEY (`id`) USING BTREE
...
CREATE TABLE `jx_order_product`  (
  ...
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 4 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '订单与商品中间表' ROW_FORMAT = Dynamic;
...
CREATE TABLE `jx_user`  (
  ...
  `balance` decimal(11, 2) NULL DEFAULT 0 COMMENT '用户余额',
